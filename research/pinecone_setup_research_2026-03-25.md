# Pinecone Setup Research — Austin's Use Case
**Date:** 2026-03-25
...
- **Live index check** — attempted `list-indexes` via MCP. Blocked by sandbox permission gate. Austin must run this manually or grant MCP permission in session. No existing indexes confirmed or denied.
...
| Model | Type | Best For |
|---|---|---|
| `llama-text-embed-v2` | Dense | **Recommended for Austin's use case.** Long passages, structured documents, ranked retrieval. Trained on diverse text corpora. Best for textbooks and arXiv papers. |
...
**Do you need to bring your own embeddings?** No — the `create-index-for-model` path handles it. If you wanted to use OpenAI `text-embedding-3-large` or a custom fine-tuned model, you would use the lower-level `create-index` (not in the MCP tools) and upsert raw vectors. For Austin's use case, Pinecone's integrated models are sufficient.
...
- Austin has 100-200GB of PDFs. A typical 500-page textbook PDF is 5-20MB → could be 5,000-40,000 books. Free tier will NOT cover the full corpus.
...
**This is the biggest engineering challenge. No single source covers this exactly for Austin's corpus — combining best practices from LlamaIndex, LangChain, and Pinecone's own documentation.**
...
- Permission gates: the MCP tools require explicit grant in the Claude Code session (as encountered in this research session — `list-indexes` was blocked by sandbox). Austin needs to grant Pinecone MCP tool access when prompted.
...
## Key Decisions Needed (Austin)

1. **Chunk size:** 512 tokens recommended — but Austin should validate with 3-5 test queries on known material before committing
...
- [ ] Existing indexes in Austin's Pinecone account (requires MCP permission grant)
