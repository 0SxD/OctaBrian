...
> Fresh GitHub clones into `C:\apps_ai\_repos\trading\`
...
1. **mctx is NOT a DeepMind repo clone** — `mctx_module_registry` in trading bot is Austin's own local registry (MCTS pre-selection atomic sweeps). No external clone needed.
...
4. **All trading repos now in `_repos/trading/`** — organized per Austin's request.
